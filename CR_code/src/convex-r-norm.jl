...
function eval_obj(M, X, relThreshold)
    finalRes = 0
    userNum = size(X)[1]
...
# TODO
function get_convex_r_norm_gradient(userRowM, posItemIdxs, negItemIdxs)
    finalGradient = 0
    # userNum = size(X,1)
    ni = length(posItemIdxs) + length(negItemIdxs)
    @assert ni != 0
    for posItemIdx in posItemIdxs
        ...
        curRHeight = get_reverse_height_convex(userRowM, curPosItemVal, negItemIdxs)
        tempSum = 0
